...
@section('content')
    <x-card>
        ...
        <form method="POST" id="add_transaksi" action="{{ route('kasir.transaksi.store') }}">
            @csrf
            @if ($errors->any())
                        ...
                        <select name="id_outlet" id="outlet" class="form-select">
                            @foreach ($outlet as $outlet)
                                <option value="{{ $outlet->id }}"
                                    {{ old('id_outlet', Auth::user()->id_outlet) == $outlet->id ? 'selected' : '' }}>
                                    {{ $outlet->nama }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        <select name="id_user" id="user" class="form-select">
                            @foreach ($users as $users)
                                <option value="{{ $users->id }}"
                                    {{ old('id_user', Auth::user()->id) == $users->id ? 'selected' : '' }}>
                                    {{ $users->name }}</option>
                            @endforeach
                        </select>
                    </div>
