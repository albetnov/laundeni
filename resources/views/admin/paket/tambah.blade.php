...
@section('content')
    <x-card>
        ...
        <form method="POST" id="add_paket" action="{{ route('admin.paket.store') }}">
            @csrf
            @if ($errors->any())
                        ...
                        <select class="form-select" name="id_outlet">
                            @foreach ($outlet as $outlet)
                                <option value="{{ $outlet->id }}"
                                    {{ old('id_outlet') == $outlet->id ? 'selected' : '' }}>{{ $outlet->nama }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="mb-2">
                        ...
                        <select name="jenis" class="form-select">
                            ...
                            <option value="bed_cover" {{ old('jenis') == 'bed_cover' ? 'selected' : '' }}>Bed Cover
                            </option>
                            <option value="kaos" {{ old('jenis') == 'kaos' ? 'selected' : '' }}>Kaos</option>
                            <option value="dll" {{ old('jenis') == 'dll' ? 'selected' : '' }}>DLL</option>
                        </select>
                    </div>
                </div>
                <div class="col">
                    <div class="mb-2">
                        ...
                        <input type="text" name="nama_paket" value="{{ old('nama_paket') }}" id="nama_paket"
                            class="form-control">
                    </div>
                    <div class="mb-2">
                        <label for="harga">Harga</label>
                        <input type="number" name="harga" value="{{ old('harga') }}" id="harga" class="form-control">
                    </div>
                </div>
            </div>
        </form>
